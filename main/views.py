...
def inquire_listing_using_email(request,id):
    ...
    try:
        ...
        emailMessage = f'Hi {listing.seller.users.username}, {request.user.username} is interested in your {listing.model} listing on Autocode'
        send_mail(emailSubject, emailMessage, 'noreply@autocode.com', [listing.seller.users.email], fail_silently=True)
        return JsonResponse({
            "success": True
        })
    except Exception as e:
